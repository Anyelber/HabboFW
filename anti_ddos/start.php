...
if(isset($_SESSION['standby'])){
	...
	if ($ad_end_defense and $ad_end_defense> $ad_date) {
		...
		$final_ad_sec = ltrim($final_ad_sec,"0");

		if(strlen($final_ad_sec) == 0){
			$final_ad_sec = 0;
		}

	
		$ad_string = ($ad_num_query >= $ad_ddos_query) ? '<?php $ad_end_defense='.safe_print($ad_date + $ad_defense_time).'; ?>' : '<?php $ad_num_query='. safe_print($ad_num_query) .'; $ad_sec_query='. $final_ad_sec .'; $text = "test";?>';
	...
	?>
		<style type="text/css">
			...
			.loading__dots__dot {background-color: #ff0000; width: 20px; height: 20px; border-radius: 50%; margin: 0 5px; color: #aa0000; } 
	...
	<div class="loading" style="margin-top: 11%;">
		...
		<div class="loading__msg" style="font-family:helvetica;">
			<center>
				<?php
					$frases = array(
						
						'¡Me gusta tu camiseta',
						'No mires atras, podrias asustarte',
						'¿Cual es el peso de la materia total del espacio?... tranquil@ nosotros tampoco lo sabemos',
						'¿Está ladrando tu perro?',
						'¿Porque las habitaciones tienen 4 paredes? ¿eso es raro no?',
						'Las pizzas son redondas, su caja es cuadrada y sus porciones triangulares, nuestro mundo está loco',
						'¿Habbo?... ¿Que es habbo?... Aaaah.... ¡Ese Habbo!'

					);
				?>
				
				<br><br>
				<?php echo $frases[rand(0,(sizeof($frases)-1))]; ?>
			 <br> 
			 <a href="https://github.com/sanix-darker" target="_blank"></a> </center></div>
	</div>
