...
if(isset($pagar)){
  if(is_admin_or_more(rol($_SESSION['id']))){
    ...
    $q = $pdo->prepare("INSERT INTO ascensos (id_dio, id_recibe, old_rol, new_rol, created_at, tipo) VALUES (:id_dio, :id_recibe, :old_rol, :new_rol, NOW(), 3)");
    $q->execute([
        $_SESSION['id'],
        $id,
        $antiguo_rol,
    ...
    $qf2 = $pdo->prepare("UPDATE users SET save_acumulado = 0, tipo_acumulado = 0 WHERE id = :id");
    $qf2->execute([
      $id
    ]);
          ...
          <div class="col-md-6">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Pagar Save con Ascenso</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
